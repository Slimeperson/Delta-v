shuttle-pilot-start = Пілотувати корабель
shuttle-pilot-end = Перестати пілотувати

shuttle-console-in-ftl = Неможливо виконати FTL коли вже у FTL!
shuttle-console-mass = Завеликий для FTL
shuttle-console-prevent = Ви не можете керувати цим кораблем.

# NAV

shuttle-console-display-label = Дисплей

shuttle-console-position = Позиція:
shuttle-console-position-value = ftldisk {$X}
shuttle-console-orientation = Орієнтація:
shuttle-console-orientation-value = {$angle}
shuttle-console-linear-velocity = Лінійна швидкість:
shuttle-console-linear-velocity-value = ftldisk {$X}
shuttle-console-angular-velocity = Кутова швидкість:
shuttle-console-angular-velocity-value = {$angularVelocity}

shuttle-console-unknown = Невідомо
shuttle-console-iff-label = {$name}({$distance}м)
shuttle-console-exclusion = Зона відчуження

shuttle-console-nav-settings = Налаштування
shuttle-console-iff-toggle = Перемкнути IFF
shuttle-console-dock-toggle = Перемкнути доки

# MAP

shuttle-console-ftl-label = Статус FTL
shuttle-console-ftl-state-Available = в наявності
shuttle-console-ftl-state-Starting = Запуск
shuttle-console-ftl-state-Travelling = Подорожі
shuttle-console-ftl-state-Arriving = Прибуття
shuttle-console-ftl-state-Cooldown = Кулдаун
shuttle-console-ftl-state-Invalid = Недійсний

shuttle-console-map-settings = Налаштування
shuttle-console-ftl-button = FTL
shuttle-console-map-rebuild = Сканувати об'єкти
shuttle-console-map-beacons = Показати маяки

shuttle-console-no-signal = Немає сигналу

shuttle-console-map-objects = Секторні об'єкти

# DOCK
shuttle-console-docked = Прикріплені об'єкти

shuttle-console-view = Переглянути
shuttle-console-undock = Відстикувати
shuttle-console-dock = Док
shuttle-console-docks-label = Доки

shuttle-console-undock-fail = Не вдалося від’єднати
shuttle-console-dock-fail = Помилка стикування